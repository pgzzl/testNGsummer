...
public class MeetingRoomDomain {
  public String id;
  ...
  public MeetingRoomDomain() {
  }

  public MeetingRoomDomain(String id, String roomName, String roomFacility, Integer roomState,
      Integer roomCapacity, String roomFloor, String roomExplain, Date gmtUpdate, Date gmtCreate) {
    this.id=id;
  ...
  public static MeetingRoomDomain buildMapper(MeetingRoom obj) {
    MeetingRoomDomain mapper = new MeetingRoomDomain();
    mapper.id=obj.getId();
  ...
  public static MeetingRoomTO convert(MeetingRoom from, MeetingRoomTO to) {
    if (from == null) return to;
    if (to == null) to = new MeetingRoomTO();
    to.setId(from.getId());
    to.setRoomName(from.getRoomName());
    to.setRoomFacility(from.getRoomFacility());
    to.setRoomState(from.getRoomState());
    to.setRoomCapacity(from.getRoomCapacity());
    to.setRoomFloor(from.getRoomFloor());
    to.setRoomExplain(from.getRoomExplain());
    to.setGmtUpdate(from.getGmtUpdate());
    to.setGmtCreate(from.getGmtCreate());
    return to;
  }

  public static MeetingRoom convert(MeetingRoomTO from, MeetingRoom to) {
    if (from == null) return to;
    if (to == null) to = new MeetingRoom();
    to.setId(from.getId());
    to.setRoomName(from.getRoomName());
    to.setRoomFacility(from.getRoomFacility());
    to.setRoomState(from.getRoomState());
