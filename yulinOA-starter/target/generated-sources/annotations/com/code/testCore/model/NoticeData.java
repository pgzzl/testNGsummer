...
public class NoticeData {
  ...
  public NoticeData(String id, String noticeTitle, String issuer, Date issueTime, String accessory,
      String detail, String noticeImg, Date gmtUpdate, Date gmtCreate) {
    this.id=id;
  ...
  public static NoticeData buildMapper(Notice obj) {
    NoticeData mapper = new NoticeData();
    mapper.id=obj.getId();
  ...
  public static NoticeDO convert(Notice from, NoticeDO to) {
    if (from == null) return to;
    if (to == null) to = new NoticeDO();
    to.setId(from.getId());
    to.setNoticeTitle(from.getNoticeTitle());
    to.setIssuer(from.getIssuer());
    to.setIssueTime(from.getIssueTime());
    to.setAccessory(from.getAccessory());
    to.setDetail(from.getDetail());
    to.setNoticeImg(from.getNoticeImg());
    to.setGmtUpdate(from.getGmtUpdate());
    to.setGmtCreate(from.getGmtCreate());
    return to;
  }

  public static Notice convert(NoticeDO from, Notice to) {
    if (from == null) return to;
    if (to == null) to = new Notice();
    to.setId(from.getId());
    to.setNoticeTitle(from.getNoticeTitle());
    to.setIssuer(from.getIssuer());
    to.setIssueTime(from.getIssueTime());
