...
public class ParkParkDomain {
  ...
  public ParkParkDomain() {
  }

  public ParkParkDomain(String id, String parkName, Integer allStall, String place,
  ...
  public static ParkParkDomain buildMapper(ParkPark obj) {
    ParkParkDomain mapper = new ParkParkDomain();
    mapper.id=obj.getId();
  ...
  public static ParkParkTO convert(ParkPark from, ParkParkTO to) {
    if (from == null) return to;
    if (to == null) to = new ParkParkTO();
    to.setId(from.getId());
    to.setParkName(from.getParkName());
    to.setAllStall(from.getAllStall());
    to.setPlace(from.getPlace());
    to.setResidueStall(from.getResidueStall());
    to.setGmtUpdate(from.getGmtUpdate());
    to.setGmtCreate(from.getGmtCreate());
    return to;
  }

  public static ParkPark convert(ParkParkTO from, ParkPark to) {
    if (from == null) return to;
    if (to == null) to = new ParkPark();
    to.setId(from.getId());
    to.setParkName(from.getParkName());
    to.setAllStall(from.getAllStall());
    to.setPlace(from.getPlace());
